...
export async function POST(request: NextRequest) {
  ...
  if (!profile?.is_admin) {
    ...
    if (!perms?.['working-days-by-client']) {
      return NextResponse.json({ error: 'You do not have access to the Working Days by Client report.' }, { status: 403 })
    }
  }

  try {
    const proxyRes = await fetch(`${process.env.RAILWAY_PROXY_URL}/report/working-days-by-client`, {
      method: 'POST',
      headers: {
